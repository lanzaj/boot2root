...
- We found online a little c program that does just that, by using thread in loops in order to provoke the data race situation needed to overwrite the read only file `/etc/passwd` (see `dirty.c` in `resources` folder)
